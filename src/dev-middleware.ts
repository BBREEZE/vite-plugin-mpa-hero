...
export const devServerMiddleware = (entries: EntryList, opt: MakeRequired<MPAHeroPluginOption>, server: ViteDevServer) => {
  return async (
    ...
    if(req.url=== '/_index'){
      let htmlTemplate = devIndexTemplate(
        renderDirectoryTree(
          buildDirectoryTree(entries.map((item: any) => ({entryPath: item.entryPath, info: item}))),
          opt.scanFileName
        )
      )
    ...
    for (let index = 0; index < entries.length; index++) {
      if(entries[index].virtualTemplateFileRelativePath === url){
        const templateContent = fs.readFileSync(path.resolve(entries[index].templatePath, `${opt.templateName}.html`), 'utf-8')
        const template = templateContent.replace('</body>', `<script type="module" src="/mpa-hero/${entries[index].entryPath}"></script></body>`)
